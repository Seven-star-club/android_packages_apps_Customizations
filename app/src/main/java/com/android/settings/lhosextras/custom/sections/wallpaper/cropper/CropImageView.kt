...
// - Sun Tsu,
// "The Art of War"
package com.android.settings.lhosextras.custom.sections.wallpaper.cropper
...
import com.android.settings.lhosextras.custom.sections.wallpaper.cropper.utils.*
import com.android.settings.lhosextras.custom.sections.wallpaper.cropper.utils.CropOverlayView.CropWindowChangeListener
...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    val cropPoints: FloatArray
        get() {
            ...
            for (i in points.indices) {
                points[i] = points[i] * mLoadedSampleSize.toFloat()
            }
            return points
        }
    ...
    fun resetCropRect() {
        ...
        applyImageMatrix(width.toFloat(), height.toFloat(), center = false, animate = false)
    ...
    fun rotateImage(degrees: Int) {
        ...
        if (mBitmap != null) {
            ...
            applyImageMatrix(width.toFloat(), height.toFloat(), center = true, animate = false)
            mImageMatrix.mapPoints(BitmapUtils.POINTS2, BitmapUtils.POINTS)
    ...
    fun flipImageHorizontally() {
        ...
        applyImageMatrix(width.toFloat(), height.toFloat(), true, animate = false)
    }
    ...
    private fun applyImageMatrix(width: Float, height: Float, center: Boolean, animate: Boolean) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (center) {
                ...
                mZoomOffsetY =
                    if (height > BitmapUtils.getRectHeight(mImagePoints)) 0f else Math.max(
                        Math.min(
